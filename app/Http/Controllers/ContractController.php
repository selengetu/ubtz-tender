...
class ContractController extends Controller {
    ...
    public function saveContract(Request $request)
    {
        ...
        if($request->contractid ==  null){
            ...
            values
            ('$request->contractno' , TO_DATE('$request->contract_date', 'yyyy-mm-dd'), '$request->contract_duration_days',  TO_DATE('$request->contract_end_date', 'yyyy-mm-dd'), TO_DATE('$request->contract_payment_date', 'yyyy-mm-dd') ,'$request->contractor_name', '$contract_amount',
           ...
           '$request->supplier_days','$request->supplier_name','$request->contract_condition','$request->contract_clarification','$request->contract_conclusion','$request->contract_reminder','$request->contract_tender')");
            return 1;
                ...
                'contract_payment_date' => $request->contract_payment_date,'contractor_name' => $request->contractor_name,'contract_amount' => $contract_amount,'currency' => $request->currency,'performance_note' => $request->performance_note,'performance_percent' => $request->performance_percent
